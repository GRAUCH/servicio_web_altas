<legend
	style="font-size: 15px; font-weight: bold; border-bottom: none; margin-bottom: 0px;">Garántias
	póliza</legend>
<!-- block -->
<div  style="border: 1px solid rgb(204, 204, 204);">

	<table aria-describedby="example_info" class="table dataTable"
		id="example">
		<thead>
			<tr role="row" style="background-color: rgb(206, 227, 246);">
				<th style="width: 20; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Póliza</th>
				<th style="width: 50px; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Garantia</th>
				<th style="width: 50px; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Garantia</th>
				<th style="width: 50px; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Capital</th>
				<th style="width: 80px; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Vto.
					Garantia</th>
				<th style="width: 80px; font-size: 12px;" colspan="1" rowspan="1"
					aria-controls="example" tabindex="0" role="columnheader">Vto.
					Cobro</th>
			</tr>
		</thead>

		<tbody aria-relevant="all" aria-live="polite" role="alert">

			<g:each in="${garantias}" var="garantia" status="i">
				<tr class="\${(i % 2) == 0 ? 'gradeA even' : 'gradeA odd'}">
				<tr>
					<td style="font-size: 10px;">
						${garantia.numPoliza}
					</td>
					<td style="font-size: 10px;">
						${garantia.codGarantia}
					</td>
					<td style="font-size: 10px;">
						${garantia.garantia}
					</td>
					<td style="font-size: 10px;">
						${garantia.capital}
					</td>
					<td style="font-size: 10px;"><g:formatDate format="dd/MM/yyyy"
							date="${garantia.fechaVencimientoGarantia}" /></td>
					<td style="font-size: 10px;"><g:formatDate format="dd/MM/yyyy"
							date="${garantia.fechaVencimientoPrima}" /></td>
				</tr>
			</g:each>
		</tbody>
	</table>
</div>
